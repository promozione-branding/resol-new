...
const CATEGORIES = ["All products", "Polymers", "Fillers", "Additives", "Specialty"];
...
function AnimatedCard({ children, rowIndex, cardIndexInRow, className }) {
  ...
  const offScreenX = fromRight ? "70px" : "-70px";

  const delayMs = cardIndexInRow * 90;
  ...
  return (
    <div
      ...
      className={className}
      style={{
        transform: visible ? "translateX(0) translateY(0)" : `translateX(${offScreenX}) translateY(8px)`,
        opacity: visible ? 1 : 0,
        transition: visible
          ? `transform 0.55s cubic-bezier(0.22, 1, 0.36, 1) ${delayMs}ms,
             opacity   0.4s  ease                             ${delayMs}ms`
          : "none", 
      }}
    >
      {children}
      ...
      <section className="mx-auto max-w-6xl px-6 py-10 pb-24">
        ...
        <div className="flex gap-2 flex-wrap mb-8 animate-[fadeUp_0.45s_0.2s_ease_both]">
          {CATEGORIES.map((label) => (
            <button
              ...
              data-active={activeCategory === label}
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProducts.map((product, index) => {
            ...
            const cardIndexInRow   = index % cols;

            return (
              <AnimatedCard
                key={product.slug}
                rowIndex={rowIndex}
                cardIndexInRow={cardIndexInRow}
                className={`
        ...
        {filteredProducts.length === 0 && (
          <p className="text-center text-sm text-[#7A6E5F] py-16">
